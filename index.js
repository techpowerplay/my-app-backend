...
import { BookingModel } from "./models/booking.js"; // fixed casing to match file
...
const DP_DIR = path.join(UPLOAD_ROOT, "DP"); // ensure DP directory exists as well
...
// Static assets
app.use("/Images", express.static(UPLOAD_ROOT)); // serves /Images/**/* (Aadhaar, DP, etc.)
app.use("/assets", express.static(path.join(__dirname, "assets"))); // optional

// Connect DB before starting
async function startServer() {
  ...
  app.post("/enquiry", async (req, res) => {
    ...
    try {
      ...
      console.log("✅ Enquiry sent to Google Sheet:", enquiry);
      res.status(200).json({ message: "Enquiry saved to Google Sheet." });
    } catch (err) {
      console.error("❌ Google Sheets Error:", err.message);
      res.status(500).json({ error: "Failed to save enquiry to Google Sheet." });
    }
  });
  ...
  // ---- Multer storage for Aadhaar images ----
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const isMemberBool = isMember === "true" || isMember === true || isMember === "1";

        // Validation
        if (!selectedConsole || !period || !startAt || !endAt || !parsedContact) {
        ...
        if (!["hourly", "daily"].includes(period)) {
          return res.status(400).json({ ok: false, message: "Invalid rental period." });
        }
        if (!controllersNum || controllersNum < 1 || controllersNum > 4) {
          return res.status(400).json({ ok: false, message: "Invalid controllers (1-4)." });
        }
        if (!durationNum || durationNum < 1) {
          return res.status(400).json({ ok: false, message: "Invalid duration." });
        }
        if (new Date(endAt) <= new Date(startAt)) {
          return res.status(400).json({ ok: false, message: "End must be after start." });
        }
        if (parsedGames.length > 5) {
          return res.status(400).json({ ok: false, message: "You can select at most 5 games." });
        }

        const total = computeServerTotal(controllersNum, durationNum, period, isMemberBool);
        if (!total) {
          return res.status(400).json({ ok: false, message: "Invalid pricing selection." });
        }

        const AdharImg = req.files?.AdharImg?.[0]?.filename || null;
        const PersonWithAdharImg = req.files?.PersonWithAdharImg?.[0]?.filename || null;
  ...
  app.post("/api/UpdateStatus/:id", async (req, res) => {
    try {
      ...
      const data = await BookingModel.updateOne({ _id: req.params.id }, { $set: { status } });
